...
unsigned long read_dir(int driveId, struct iso_directory_record *dir_read, char *search, char *filename, struct iso_directory_record *dir_found) {
    ...
    while (offset < read_size) {
        ...
        if ((*((char *)(dir->flags)) & IS_DIR) && (*((unsigned char *)(dir->name_len)) > 1)) {
            ...
            if (strlen(newfilename) < strlen(search) && !memcmp(search, newfilename, strlen(newfilename)) && search[strlen(newfilename)] == '/') {
...
unsigned long read_file(int driveId, struct iso_directory_record *dir_read, unsigned char *buffer, unsigned int max_bytes_to_read) {
    unsigned long read_size;
    ...
    if (max_bytes_to_read > *((unsigned int *)(dir_read->size))) {
        ...
        read_size = *(unsigned long *)max_bytes_to_read;
