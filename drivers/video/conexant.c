    ...
    if (conexant_calc_mode_params(mode, &param)) {
        ...
        regs[ADR(0x92)] = ((param.v_blanki) & 0xff);
        {
            ...
            dwPllRatio          = (int)(6.0 * ((double)param.h_clko / vidstda[mode->tv_encoding].m_dSecHsyncPeriod) *
                                            param.clk_ratio * 0x10000 / pll_base +
                                        0.5);
        ...
        {
            unsigned int dwSubcarrierIncrement = (unsigned int)((65536.0 * 65536.0) * (vidstda[mode->tv_encoding].m_dHzBurstFrequency * vidstda[mode->tv_encoding].m_dSecHsyncPeriod / (double)param.h_clko) + 0.5);
...
static int conexant_calc_mode_params(
    ...
    if (dBestMetric != invalidMetric) {
        ...
        param->v_activeo = (unsigned int)((
                                              (mode->yres * vidstda[mode->tv_encoding].m_TotalLinesOut) + param->v_linesi - 1) /
                                          param->v_linesi);
...
static void conexant_calc_blankings(
    ...
    // H_BLANKO
    param->h_blanko     = 2 * (int)(vidstda[mode->tv_encoding].m_dSecImageCentre / (2 * vidstda[mode->tv_encoding].m_dSecHsyncPeriod) *
                                        param->h_clko +
                                    0.5) -
                          mode->xres + 15;
