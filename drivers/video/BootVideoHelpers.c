...
int BootVideoOverlayCharacter(
    ...
    for (y = 0; y < nHeight; y++) {
        ...
        for (n = 0; n < nWidth; n++) {
            ...
            if (b1) {
                *pbaDest = (unsigned char)((b1 * (rgbaColourAndOpaqueness & 0xff)) >> 4);
                pbaDest++;
                *pbaDest = (unsigned char)((b1 * ((rgbaColourAndOpaqueness >> 8) & 0xff)) >> 4);
                pbaDest++;
                *pbaDest = (unsigned char)((b1 * ((rgbaColourAndOpaqueness >> 16) & 0xff)) >> 4);
                pbaDest++;
                *pbaDest++ = 0xff;
            } else {
                pbaDest += 4;
...
void BootVideoClearScreen(JPEG *pJpeg, int nStartLine, int nEndLine) {
    ...
    if (nEndLine >= vmode.height)
        nEndLine = vmode.height - 1;
