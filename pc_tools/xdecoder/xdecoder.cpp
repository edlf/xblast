...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        ...
        for (count = 0; count < 0x3000; count += 9) {
            ...
            if (!xcodeString.compare("xcode_outb") || !xcodeString.compare("xcode_inb")) { // Check for SMBUS string swap
            ...
            if (!outLine5.compare("xcode_outb(SMBUS, 0x00000010);") && !outLine4.compare("xcode_ifgoto(0x00000010, 0xffffffff);") && !outLine3.compare("xcode_inb(SMBUS);") && !outLine2.compare("xcode_outb(SMBUS+2, 0x0000000a);") && outLine1.find("xcode_outb(SMBUS+6, ") != std::string::npos && outLine0.find("xcode_outb(SMBUS+8, ") != std::string::npos) {
